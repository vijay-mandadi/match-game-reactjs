...
class MatchGame extends Component {
  ...
  componentDidMount() {
    this.intervalId = setInterval(this.renderTime, 1000)
  }
  ...
  getFilteredItems = () => {
    ...
    const filtered = imagesList.filter(each => each.category === active)
  ...
  thumbnailMatch = id => {
    ...
    const match = imagesList.filter(each => each.id === id)
    const check = match[0].imageUrl === image
    if (check) {
      const newimage =
        imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
      this.setState(prev => ({score: prev.score + 1, image: newimage}))
    } else {
  ...
  render() {
    const {active, image, timer, isRunning, score} = this.state
    const filteredImages = this.getFilteredItems()
    return (
      <div className="bg-container">
        ...
        {isRunning ? (
          <div className="game-container">
            <img src={image} alt="match" className="image" />
            ...
            <ul className="image-container">
              {filteredImages.map(each => (
                <ProjectItem
                  key={each.id}
                  thumbnailMatch={this.thumbnailMatch}
                  imageItem={each}
                />
              ))}
            </ul>
          ...
          <div className="gameover-container">
            ...
            <button type="button" className="reset" onClick={this.playAgain}>
              <img
